...
def start_bot(new_token, bot_id):
    ...
    bot_id = str(bot_id)  # Garante que é string
    if not bot_id in processes.keys():
        ...
        bots_data[bot_id] = bot_obj  # Não precisa str() aqui pois bot_id já é string
        processes[bot_id] = process   # Não precisa str() aqui pois bot_id já é string
...
def ban_bot(bot_id):
    if session.get("auth", False):
        ...
        if bot:
            ...
            # 4. Marca bot como banido no banco ANTES de notificar
            manager.save_bot_token_backup(bot_id, bot_token)  # ADICIONE ESTA LINHA
...
def unban_owner(owner_id):
    if session.get("auth", False):
        try:
            ...
            if REGISTRO_TOKEN:
                try:
                    message = (
                        ...
                        f"<b>Bots restaurados:</b> {len(bots_to_restart)}\n\n"
                        "Seus bots foram reiniciados automaticamente.\n"
            ...
            return jsonify({
                "success": True, 
                "message": f"Owner desbanido e {len(bots_to_restart)} bots reiniciados"
            })
        except Exception as e:
